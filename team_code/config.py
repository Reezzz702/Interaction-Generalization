...
class GlobalConfig:
  ...
  def __init__(self):
    ...
    #TF+
    self.hdmap_resolution_width  = 160 # Width resoultion the hdmap loss is upsampled to. Double check if width and height are swapped if you want to make them non symmetric.
    self.hdmap_resolution_height = 160 # Height resoultion the hdmap loss is upsampled to. Double check if width and height are swapped if you want to make them non symmetric.
    self.temp_hdmap = None
    ...
    self.root_dir = '/media/hcis-s02/disk2/sanity_dataset'  # Dataset root dir
    ...
    self.detailed_loss_weights = {
        ...
        'loss_forcast': 0.2,
        'loss_selection': 0.0,
        'loss_vehicle_occupancy': 1.0,
        'loss_pedestrian_occupancy': 1.0
    }
    self.root_dir = '/media/hcis-s02/disk2/sanity_dataset_new'
    self.train_towns = []
    self.val_towns = []
    self.train_data = []
    self.val_data = []
    ...
    # stp3 loss
    self.vehicle_weights = [1.0, 2.0]
    self.vehicle_use_top_k = True
    self.vehicle_top_k_ratio = 0.25
    self.pedestrian_weights = [1.0, 10.0]
    self.pedestrian_use_top_k = True
    self.pedestrian_top_k_ratio = 0.25
  ...
  def initialize(self, root_dir='', setting='all', **kwargs):
    ...
    if setting == 'all':
      first_val_town = 'Town10HD'
      second_val_town = 'Town10HD'
